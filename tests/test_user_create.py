...
from api_methods.user import CreateUser as C, LoginUser as L, DeleteUser as D, TokenUser as T

class TestUser:
    @allure.title('Создание пользователя')
    @allure.description("Проверка успешного создания пользователя")
    def test_create_user_success(self):
        response, payload = C.register_new_user()
    ...
    def test_create_user_duplicate_check_error_message(self):
        response_first, payload = C.register_new_user()
        
        response_second, _ = C.register_new_user(payload)
        assert response_second.status_code == code.FORBIDDEN, f'Ошибка! Ожидаемый код статуса {code.FORBIDDEN}, фактический {response.status_code}'

        if response_first.status_code == code.OK:
            # Очистка после теста
            response_auth = L.get_auth(payload)
            access_token = T.get_access_token(response_auth)
            D.delete_user_by_token(access_token)
        
        message = response_second.json()
    ...
    def test_create_user_with_empty_field_check_error_message(self, payload):
        response, _ = C.register_new_user(payload)
        ...
        assert len(message) == 2  # проверяем, что в ответе только два поля
